...
static char *findattr(int attr, const char *p, const char *e, size_t *len) {
  while (p < e) {
    struct nlattr nla;
...
static uint16_t nl80211fam(void) {
  static const char family[] = "nl80211";
  static uint16_t id;
  ssize_t r;
  size_t len;
  char ctrl[NLMSG_HDRLEN + GENL_HDRLEN + NLA_HDRLEN +
            NLA_ALIGN(sizeof(family))] = {0},
            *p = ctrl;
...
static int ifindex(const char *interface) {
...
const char *wifi_essid(const char *interface) {
  ...
  ssize_t r;
  size_t len;
  char req[NLMSG_HDRLEN + GENL_HDRLEN + NLA_HDRLEN + NLA_ALIGN(4)] = {0},
                                                     *p = req;
  ...
  if ((size_t)r <= NLMSG_HDRLEN + GENL_HDRLEN)
    return NULL;
  p = findattr(NL80211_ATTR_SSID, resp + NLMSG_HDRLEN + GENL_HDRLEN, resp + r,
               &len);
  if (p)
    p[len] = 0;
...
const char *wifi_perc(const char *interface) {
  ...
  while (1) {
    r = recv(nlsock, resp, sizeof(resp), 0);
    if (r < 0 || (size_t)r < sizeof(hdr))
      return NULL;

    for (p = resp; p != resp + r && (size_t)(resp + r - p) >= sizeof(hdr);
         p = e) {
      memcpy(&hdr, p, sizeof(hdr));
      e = p + hdr.nlmsg_len;

      if (hdr.nlmsg_len > NLMSG_HDRLEN + GENL_HDRLEN) {
  ...
  snprintf(result, sizeof(result), "%s", icon);
  return result;
}
